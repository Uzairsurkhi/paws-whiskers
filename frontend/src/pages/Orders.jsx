...
export const OrderCard = ({ order, admin = false, onCancel, children }) => {
  const cancellable = !admin && onCancel && ["new", "packed"].includes(order.fulfillment_status);
        ...
        <div className="flex items-center gap-3">
          ...
          {cancellable && (
            <button
              ...
              className="rounded-full border border-red-200 bg-red-50 px-3 py-1 text-xs font-semibold text-red-700 transition-colors hover:bg-red-100"
            >
              Cancel order
            </button>
          )}
...
export default function Orders() {
  ...
  const handleCancel = async (orderId) => {
    if (!window.confirm("Are you sure you want to cancel this order? A confirmation email will be sent.")) {
      return;
    }
    try {
